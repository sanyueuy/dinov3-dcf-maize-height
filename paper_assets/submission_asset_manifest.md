...
- figures/Figure_4_DINOv3_DCF_zero_shot_workflow.png and .pdf: Fig. 4. Mind-map workflow for DATA325 and DINOv3-DiffCorn-Fusion (DINOv3-DCF). The benchmark, frozen tokens, pooling equation, camera-height conditioning, structured latent head, zero-shot metrics, and failure diagnostics are shown as connected branches.
...
- figures/Figure_10_Real_photo_DATA325_prediction_matrix.png and .pdf: Fig. 10. Real-photo DATA325 prediction matrix. Each case shows a manual ground-truth box on the original image, the ROI used for model prediction, and a deterministic height-error diagnostic. "Difference" denotes plant-height error, foreground mask, and failure category; it is not a pixel-level segmentation or 3D point-cloud difference.
...
- supplementary_figures/Supplementary_Figure_5_ROI_contamination_morphometric_diagnostics.png and .pdf: Supplementary Fig. 5. ROI contamination and morphometric diagnostics. Foreground/background fractions from deterministic color-index masks are weakly correlated with absolute error, while source-trained bbox/mask baselines quantify the strength and limits of simple agricultural geometry.
- supplementary_figures/Supplementary_Figure_6_Qualitative_stage_attention_error_gallery.png and .pdf: Supplementary Fig. 6. Qualitative stage-wise and attention/error gallery. Real DATA325 examples combine low-, mid-, and tall-stage cases with success, over-estimation, under-estimation, sparse-plant, and high-uncertainty ROIs.
- supplementary_figures/Supplementary_Figure_7_Diagnostic_negative_controls.png and .pdf: Supplementary Fig. 7. Diagnostic negative controls. Camera-height correction, bbox geometry, feature-statistic alignment, and DANN do not remove the external-greenhouse gap, so the remaining error is not a single metadata or marginal-alignment artifact.
- supplementary_figures/Supplementary_Figure_8_Extended_DATA325_real_photo_matrix.png and .pdf: Supplementary Fig. 8. Extended real-photo DATA325 matrix covering successful predictions, over-estimation, under-estimation, early sparse plants, high-background ROIs, and high TTA uncertainty.
